...
import { getConversation, saveConversation, generateId, deleteConversation, getSettings, getAccounts } from '../lib/storage';
import {
  chatWithOpenRouter, AGENT_SYSTEM_PROMPT, RESPONSE_SYSTEM_PROMPT,
  fetchZenrioAccounts, connectZenrioAccount, disconnectZenrioAccount,
  createPost, listPosts, getPost, editPost, deletePost,
  schedulePost, publishPost, unpublishPost, retryPost,
  getComments, replyToComment, hideComment, likeComment, deleteComment,
  listConversations, getConversationMessages, replyToDM,
  getAccountHealth, getAccountFollowerStats,
  getPostAnalytics, getDailyAnalytics, getBestPostingTime,
  uploadMedia, listQueueSlots, createQueueSlot,
} from '../lib/api';
...
export default function ChatWindow() {
  ...
  const executeAction = async (
    ...
    if (!settings?.zenrioApiKey) {
      throw new Error('Zernio API key not configured. Please add it in Settings.');
    }
    ...
    switch (action) {
      ...
      case 'list_accounts': {
        ...
        let list = Array.isArray(result) ? result : result?.accounts || result?.data || [];
        let note = '';
        if (list.length === 0) {
        ...
        const summary = list.map((a: any) => `  • ${a.platform}: @${a.username || a._id}`).join('\n');
        return { success: true, message: `Connected accounts (${list.length}):\n${summary}${note}` };
      }
      case 'connect_account': {
        ...
        if (authUrl) {
          ...
          return { success: true, message: `OAuth window opened for ${platform}. Authorize and then use "list accounts" to see it.` };
        }
      ...
      case 'account_health': {
        ...
        return { success: true, message: `Account health: ${JSON.stringify(result)}` };
      }
      case 'follower_stats': {
        const result = await getAccountFollowerStats();
        return { success: true, message: `Follower stats: ${JSON.stringify(result)}` };
      }
      ...
      case 'create_post': {
        ...
        const result = await createPost(platform, content, mediaUrls, accountId);
        const postStatus = result?.post?.status || result?.status || 'published';
      ...
      case 'schedule_post': {
        ...
        const mediaUrls = metadata.mediaUrls as string[] | undefined;
        const accountId = metadata.accountId as string | undefined;
        if (!scheduledAt) throw new Error('scheduledAt (ISO datetime) is required in metadata');
        const result = await schedulePost(platform, content, scheduledAt, mediaUrls, accountId);
        const postId = result?.post?._id || result?._id || '';
        return {
          success: true,
      ...
      case 'list_posts': {
        ...
        const list = Array.isArray(result) ? result : result?.posts || result?.data || [];
        if (list.length === 0) {
        ...
        return { success: true, message: `Posts (${list.length}):\n${summary}` };
      }
      case 'get_post': {
        const postId = (metadata.postId as string) || content;
        if (!postId) throw new Error('postId is required in metadata');
        const result = await getPost(postId);
        return { success: true, message: `Post details: ${JSON.stringify(result, null, 2)}` };
      }
      case 'edit_post': {
        const postId = metadata.postId as string;
        if (!postId) throw new Error('postId is required in metadata');
        await editPost(postId, content, metadata.overrides as Record<string, unknown> || {});
      ...
      case 'get_comments': {
        ...
        return { success: true, message: `Comments: ${JSON.stringify(result)}` };
      }
      case 'reply_comment': {
        ...
        return { success: true, message: `Reply sent to comment ${commentId}.` };
      }
      case 'hide_comment': {
        const postId = metadata.postId as string;
        const commentId = metadata.commentId as string;
        if (!postId || !commentId) throw new Error('postId and commentId required in metadata');
      ...
      case 'list_conversations': {
        ...
        const convList = Array.isArray(result) ? result : result?.conversations || result?.data || [];
        ...
        const summary = convList.map((c: any) =>
          `  • ${c._id || c.id}: "${(c.subject || c.lastMessage?.text || '(no subject)').substring(0, 60)}"`
        ).join('\n');
        return { success: true, message: `Conversations (${convList.length}):\n${summary}` };
      }
      case 'get_conversation': {
        const conversationId = metadata.conversationId as string;
        if (!conversationId) throw new Error('conversationId is required in metadata');
        const result = await getConversationMessages(conversationId);
        return { success: true, message: `Messages: ${JSON.stringify(result)}` };
      }
      case 'reply_dm': {
        const conversationId = metadata.conversationId as string;
        if (!conversationId) throw new Error('conversationId is required in metadata');
        await replyToDM(conversationId, content);
        return { success: true, message: `DM sent in conversation ${conversationId}.` };
      }

      // ── Analytics ──
      case 'get_analytics': {
        const result = await getPostAnalytics(platform || undefined);
        return { success: true, message: `Analytics: ${JSON.stringify(result)}` };
      }
      case 'daily_analytics': {
        const result = await getDailyAnalytics(platform || undefined);
        return { success: true, message: `Daily analytics: ${JSON.stringify(result)}` };
      }
      case 'best_time': {
        const result = await getBestPostingTime(platform || undefined);
        return { success: true, message: `Best posting times: ${JSON.stringify(result)}` };
      }
      ...
      case 'list_queue': {
        ...
        return { success: true, message: `Queue slots: ${JSON.stringify(result)}` };
      }
